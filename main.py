...
class New_Game:
    ...
    def display_manager(self, new_block):
    ...
    def gen_empty_field(self):
        for row in range(self.rows_number):
            self.field_matrix.append([0 for col in range(self.cols_number)])
    ...
    def get_user_input(self):
        ...
        while user_input not in self.column_names:
            if user_input == 'exit':
                ...
                print('\nSelected column is not available\nPlease select one of the following options: ', ', '.join(f'{opt}' for opt in self.column_names))
                user_input = input('Choose a column to add the piece: ')
    ...
    def display_field_matrix(self):
        for row in range(self.rows_number + self.border_num):
            for col in range(self.cols_number + self.border_num):
                ...
                if col in [0, self.cols_number + (self.border_num / 2)] or row == self.rows_number + (self.border_num / 2):
                    print('@',end=' ')
                elif row == 0:
                    print(' ',end=' ')
                else:
    ...
    # Place a block lowest possible spot in selected column
    ...
    def update_column(self, col):
        '''
        while self.field_matrix[9][col] == 
        for row in self.rows_number:
        '''
        pass
    ...
    # Return block value based on possition
...
game = New_Game(number_of_rows, number_of_columns, score_to_win)
game.game_loop()